...
module RolePermissionsStrategy
  ...
  class RolePermissionsMatrix
    ...
    # New Task: anyone (non-readonly) can enter new tasks
    def new_task?
      is_all_except_readonly?
    end

    # Edit Task: anyone (non-readonly) can edit tasks (and shouldnt we track this somewhere?)
    def edit_task?(taskid)
      is_all_except_readonly?
    end

    # Assign/Change assignation of Task: only admin
    def change_assignee?(taskid)
      is_admin?
    end

    # Change status of Task: anyone (non-readonly, non-addonly)
    ...
    # New Comment: anyone (non-readonly) can enter new comments
    def new_comment?
      is_all_except_readonly?
    end

    # Edit Comment: only admins && commenter can edit comments
    def edit_comment?(commentid)
      is_admin_or_commenter?(commentid)
    end

    # Delete Comment: only admins && commenter can delete comments
