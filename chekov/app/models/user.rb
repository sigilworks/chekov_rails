class User < ActiveRecord::Base
  ...
  scope :assignees, -> { joins(:roles).where(:roles => { :id => Role.ASSIGNEE }) } # 1
  scope :reporters, -> { joins(:roles).where(:roles => { :id => Role.REPORTER }) } # 4
  scope :commenters, -> { joins(:roles).where(:roles => { :id => Role.COMMENTER }) } # 2
